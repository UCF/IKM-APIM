...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->db->select('password, salt')
		                  ...
		                  //->limit(1)
		                  ->order_by('id', 'desc')
		                  ->get($this->tables['users'],0,1);

		$hash_password_db = $query->row();
		
		$count = 0;
		foreach ($query->result_array() as $key => $row){ $count++; }
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where('username', $username)
										...
										->order_by("id", "ASC")
										//->limit(1)
		                ->count_all_results($this->tables['users'],0,1) > 0;
	}
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('default_group', 'ion_auth')),0,1)->row();
		...
		//$id = $this->db->insert_id(); /*** broken
		$query = $this->db->query("SELECT IDENT_CURRENT('".$this->tables['users']."') as last_id");
		$res = $query->result();
		$id = $res[0]->last_id;
	 ...
	 * @author Mathew
	 **/
	public function get_auth($identity,$password,$remember)
	{
		/* this is the original logic copied here mainly to support local administrator override */
		//get info from ion_auth table ecause the user has rights
					$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login')
						...
						//if ($query->num_rows() === 1)
						{
							...
							//do this only for addministrator
							if($identity == 'administrator' || $identity == 'testuser') {
								
								$password = $this->hash_password_db($user->id, $password);
					
								if ($password !== TRUE){ 
									$this->trigger_events('post_login_unsuccessful');
									$this->set_error('login_nonauth');
									return false;
									exit;
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($identity == 'administrator' || $identity == 'testuser'){
			$auth = $this->get_auth($identity,$password,$remember);
		...
		if (empty($identity) || empty($password))
		{
		    ...
		    ldap_set_option($ldap, LDAP_OPT_PROTOCOL_VERSION, 3);
			 ...
			 if ($bind=ldap_bind($ldap, $username .'@' .$domain, $password))
			 {
				...
				/**** IF the bind is successful then grant initial access.  Final auth to be checked below ***/
				
				//$filter="(sAMAccountName=$username)";
				//$result = ldap_search($ldap,"dc=MYDOMAIN,dc=COM",$filter);
				//$attr = array("memberof");
				//$result = ldap_search($ldap, $ldapconfig['basedn'], $filter, $attr) or exit("Unable to search LDAP server");
	...
	function get_attempts_num($identity)
	{
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$count = 0;
			foreach ($qres->result_array() as $key => $row){ $count++; }
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		//$group_id = $this->db->insert_id(); ** broken
		
		$query = $this->db->query("SELECT IDENT_CURRENT('".$this->tables['groups']."') as last_id");
		$res = $query->result();
		$group_id = $res[0]->last_id;
	...
	**/
	...
	protected function _filter_data($table, $data)
	{
		$filtered_data = array();
		//fix for MSSQL
		$columns_get = $this->db->query("SELECT column_name,* from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = '".$table."'");
		$columns = array();
		//$columns = $this->db->list_fields($table);
		
		foreach ($columns_get->result() as $key => $row){
