...
class General_model extends CI_Model
{
	...
	function openclose_check($user,$restricted){
		$this->db->select('*');
		$this->db->from($this->tables['users'].' a');
		$this->db->join($this->tables['usergroups']. ' b','a.id = b.user_id','inner');
		$this->db->join($this->tables['groups']. ' c','c.id = b.group_id','inner');
		$this->db->where('a.username', $user);
		$this->db->where_in('c.name', $restricted);
		$query = $this->db->get();
