...
class Main extends CI_Controller {
	...
	public function update_regional()
	{
		...
		if($VALSPS == 'true'){ $VALSPS = 1; } else { $VALSPS = 0; }
	
		//get the variables for the locations and put them into an array based on location and choice
		$region_vars = array("ALTSPRNG","COCOA","DAYTONA","LEESBURG","OCALA","PALMBAY","LAKEMARY","SOUTHLAKE","OSCEOLA","METROWEST","VALENCIA","VALSPS");
		...
		foreach ($region_result as $reg_key => $reg_row){
			...
			if($reg_key == 'VALSPS'){ $reg_key = 'VAL-SPS'; }
	...
	public function update_main(){
		...
		if (!$this->ion_auth->logged_in()){
			...
			$downtown_campus = $this->input->get('downtown');
			...
			if($ncsara == 'true'){ $ncsara = 1; } else { $ncsara = 0; }
			...
			if($downtown_campus == 'true'){ $downtown_campus = 1; } else { $downtown_campus = 0; }
			...
			/*check for editing window*/
			$openclose = $this->config->item('openclose', 'main');
			
			if ($openclose == 0){
				$check = $this->General_model->openclose_check($username,$this->config->item('restrict', 'main'));
	
				if($check->num_rows() != 0){
					$results['output'] = 'Error';
					$results['msg'] = 'APIM is currently closed to edits';
			...
			if($subplan == ''){
				...
				$batch =  array(
							...
							'Downtown_Campus' => $downtown_campus
							/*'ALTSPRNG' => $altspring,
							'COCOA' => $daytona,
							'DAYTONA' => $daytona,
							'LEESBURG' => $leesburg,
