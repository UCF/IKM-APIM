	...
	$(document).ready(function () {
		...
		var source =
		{
		    ...
		    datafields: [
				...
				{ name: 'TermStart', type: 'string' },{ name: 'Career', type: 'string' },
				{ name: 'check', type: 'number' }, { name:'StatusChange', type: 'string' },
				{ name: 'PlanLongName', type: 'string' },{ name: 'SubPlanLongName', type: 'string' },
			...
			if(prev_cellName == 'PlanLongName'){
					if(prev_cellData != rowdata.PlanLongName){
							updateAll = true;
						}
				}
			...
			updata = updata + "&flvc=" + rowdata.FLVC + "&readmit=" + rowdata.ReAdmit + "&online=" + rowdata.Online +"&orient=" + rowdata.Orientation;
		...
		$('#ponline-check').change(function(){
			alert("filter for UCF Online courses");
		});
